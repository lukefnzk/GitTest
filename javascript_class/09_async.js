...
// ------------------------------------------------------------
// 5) ★ .then() ↔ await 는 자유롭게 '섞어' 쓸 수 있다
// ------------------------------------------------------------
// 4번에서 봤듯 둘은 '같은 것'이라, 한쪽 문법으로 만든 걸 다른 쪽 문법으로 받아도 된다.
// (이게 'async/await 는 Promise 를 대체한 게 아니라 위에 얹은 문법'이라는 증거다.)
//   위 4번의 checkAgePromise(new Promise) / checkAgeAsync(async) 를 그대로 재활용한다.

// [섞기 1] async 함수가 '반환한 Promise' 를 .then() 으로 받기
//   → 만들 땐 async(값을 return), 소비는 .then() 으로.
checkAgeAsync(25).then((v) => console.log("I. async→then :", v)); // → 25세: 통과

// [섞기 2] .then() 용으로 만든 Promise 를 async 함수 안에서 await 로 받기
//   → 만들 땐 new Promise, 소비는 await 로.
async function mix() {
  // await 는 async 함수 안에서만 쓸 수 있어서 mix 를 async 로 감쌌다.
  const v = await checkAgePromise(25); // 일반 Promise 를 await 해도 잘 받아진다
  console.log("J. promise→await:", v); // → 25세: 통과
}
mix();
// ★ 정리: '만드는 쪽'과 '소비하는 쪽'의 문법이 서로 달라도 된다 —
//   둘 다 바닥은 똑같은 Promise 라서. 그래서 코드베이스에 .then 과 await 가 섞여 있어도 정상이다.

// [Python 비교] Python 도 '만든 방식'과 무관하게 awaitable 이면 await 로 받는다 —
//   여기서 익힌 '섞어 쓰기' 감각은 Python asyncio 에도 그대로 통한다.

// ------------------------------------------------------------
// 6) 실전 — fetch 로 인터넷에서 데이터 받기 (인터넷 연결 필요)
// ------------------------------------------------------------
